...
# Attach to pytest to share state between __main__ and the pytest-loaded module
if not hasattr(pytest, "COVERED_OPCODES"):
    pytest.COVERED_OPCODES = set()
...
def get_traces(opcode: str):
    ...
    pytest.COVERED_OPCODES.add(opcode)
    return traces
...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    ...
    uncovered = sorted(all_opcodes - pytest.COVERED_OPCODES)
...
if __name__ == "__main__":
    import sys
    pytest.main(["-v", __file__], plugins=[sys.modules[__name__]])